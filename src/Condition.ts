...
export type WhenOptionsFn<S> = (values: any[]) => S | undefined

export type WhenIs = boolean | number | string | WhenIsFn

export type SorFn<T, S> = S | WhenOptionsFn<S>
...
function resolveSchema<T, S>(values: any[], schemaCaller: S, sOrFn?: SorFn<T, S>): S {
  ...
  const fn: WhenOptionsFn<S> = sOrFn as any // FIXME
  const schemaResult = fn.apply(schemaCaller, (values as any).concat(schemaCaller)) // FIXME wow so confusing
